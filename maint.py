...
class maint_vehicle_cost(osv.Model):
    _name = 'maint.vehicle.cost'
    ...
    def _set_odometer(self, cr, uid, id, name, value, args=None, context=None):
        ...
        odometer_id = self.pool.get('maint.vehicle.odometer').create(cr, uid, data, context=context)
    ...
    _columns = {
        ...
        'vehicle_id': fields.many2one('maint.vehicle', 'Vehicle', required=True, help='Vehicle concerned by this log'),
        'cost_subtype_id': fields.many2one('maint.service.type', 'Type', help='Cost type purchased with this cost'),
        ...
        'parent_id': fields.many2one('maint.vehicle.cost', 'Parent', help='Parent cost to this current cost'),
        'cost_ids': fields.one2many('maint.vehicle.cost', 'parent_id', 'Included Services'),
        'odometer_id': fields.many2one('maint.vehicle.odometer', 'Odometer', help='Odometer measure of the vehicle at the moment of this log'),
        ...
        'contract_id': fields.many2one('maint.vehicle.log.contract', 'Contract', help='Contract attached to this cost'),
    ...
    def create(self, cr, uid, data, context=None):
        ...
        if 'contract_id' in data and data['contract_id']:
            contract = self.pool.get('maint.vehicle.log.contract').browse(cr, uid, data['contract_id'], context=context)
        ...
        if 'odometer' in data and not data['odometer']:
            ...
            del(data['odometer'])
        return super(maint_vehicle_cost, self).create(cr, uid, data, context=context)


class maint_vehicle_tag(osv.Model):
    _name = 'maint.vehicle.tag'
    _columns = {
...
class maint_vehicle_state(osv.Model):
    _name = 'maint.vehicle.state'
    _order = 'sequence asc'
    _columns = {
        'name': fields.char('Name', required=True),
        'sequence': fields.integer('Sequence', help="Used to order the note stages")
    }
    _sql_constraints = [('maint_state_name_unique','unique(name)', 'State name already exists')]



class maint_vehicle_model(osv.Model):
    ...
    def on_change_brand(self, cr, uid, ids, model_id, context=None):
        ...
        brand = self.pool.get('maint.vehicle.model.brand').browse(cr, uid, model_id, context=context)
        return {
            'value': {
...
class maint_vehicle_model_brand(osv.Model):
    _name = 'maint.vehicle.model.brand'
...
class maint_vehicle(osv.Model):
    ...
    def return_action_to_open(self, cr, uid, ids, context=None):
        ...
        if context.get('xml_id'):
            res = self.pool.get('ir.actions.act_window').for_xml_id(cr, uid ,'maint', context['xml_id'], context=context)
            res['context'] = context
    ...
    def act_show_log_cost(self, cr, uid, ids, context=None):
        ...
        if context is None:
            context = {}
        res = self.pool.get('ir.actions.act_window').for_xml_id(cr, uid ,'maint','maint_vehicle_costs_act', context=context)
        res['context'] = context
        res['context'].update({
    ...
    def _get_odometer(self, cr, uid, ids, odometer_id, arg, context):
        ...
        for record in self.browse(cr,uid,ids,context=context):
            ids = self.pool.get('maint.vehicle.odometer').search(cr, uid, [('vehicle_id', '=', record.id)], limit=1, order='value desc')
            if len(ids) > 0:
                res[record.id] = self.pool.get('maint.vehicle.odometer').browse(cr, uid, ids[0], context=context).value
        return res
    ...
    def _set_odometer(self, cr, uid, id, name, value, args=None, context=None):
        if value:
            ...
            return self.pool.get('maint.vehicle.odometer').create(cr, uid, data, context=context)
            ...
            for element in record.log_contracts:
                if element.state in ('open', 'toclose') and element.expiration_date:
                    ...
                    if overdue or due_soon:
                        ids = self.pool.get('maint.vehicle.log.contract').search(cr,uid,[('vehicle_id', '=', record.id), ('state', 'in', ('open', 'toclose'))], limit=1, order='expiration_date asc')
                        if len(ids) > 0:
                            #we display only the name of the oldest overdue/due soon contract
                            name=(self.pool.get('maint.vehicle.log.contract').browse(cr, uid, ids[0], context=context).cost_subtype_id.name)
    ...
    def _get_default_state(self, cr, uid, context):
        try:
            model, model_id = self.pool.get('ir.model.data').get_object_reference(cr, uid, 'maint', 'vehicle_state_active')
        except ValueError:
            model_id = False
        return model_id
    ...
    _columns = {
        ...
        'log_services': fields.one2many('maint.vehicle.log.services', 'vehicle_id', 'Services Logs'),
        'log_contracts': fields.one2many('maint.vehicle.log.contract', 'vehicle_id', 'Contracts'),
    ...
    def on_change_model(self, cr, uid, ids, model_id, context=None):
        if not model_id:
            return {}
        model = self.pool.get('maint.vehicle.model').browse(cr, uid, model_id, context=context)
        return {
            'value': {
    ...
    def create(self, cr, uid, data, context=None):
        ...
        vehicle_id = super(maint_vehicle, self).create(cr, uid, data, context=context)
        vehicle = self.browse(cr, uid, vehicle_id, context=context)
        self.message_post(cr, uid, [vehicle_id], body=_('%s %s has been added!') % (vehicle.model_id.name,vehicle.license_plate), context=context)
        return vehicle_id
    ...
    def write(self, cr, uid, ids, vals, context=None):
        ...
        for vehicle in self.browse(cr, uid, ids, context):
            ...
            if 'model_id' in vals and vehicle.model_id.id != vals['model_id']:
                value = self.pool.get('maint.vehicle.model').browse(cr,uid,vals['model_id'],context=context).name
                oldmodel = vehicle.model_id.name or _('None')
                changes.append(_("Zone: from '%s' to '%s'") %(oldmodel, value))
            if 'driver_id' in vals and vehicle.driver_id.id != vals['driver_id']:
                ...
                changes.append(_("Ashram Manager: from '%s' to '%s'") %(olddriver, value))
            if 'state_id' in vals and vehicle.state_id.id != vals['state_id']:
                value = self.pool.get('maint.vehicle.state').browse(cr,uid,vals['state_id'],context=context).name
            ...
            if 'license_plate' in vals and vehicle.license_plate != vals['license_plate']:
                ...
                changes.append(_("Ashram Name: from '%s' to '%s'") %(old_license_plate, vals['license_plate']))

            if len(changes) > 0:
                self.message_post(cr, uid, [vehicle.id], body=", ".join(changes), context=context)

        vehicle_id = super(maint_vehicle,self).write(cr, uid, ids, vals, context)
        return True


class maint_vehicle_odometer(osv.Model):
    _name='maint.vehicle.odometer'
    ...
    def on_change_vehicle(self, cr, uid, ids, vehicle_id, context=None):
        if not vehicle_id:
            return {}
        odometer_unit = self.pool.get('maint.vehicle').browse(cr, uid, vehicle_id, context=context).odometer_unit
        return {
            'value': {
                'unit': odometer_unit,
            }
    ...
    _columns = {
        ...
        'vehicle_id': fields.many2one('maint.vehicle', 'Vehicle', required=True),
...
class maint_vehicle_log_fuel(osv.Model):

    def on_change_vehicle(self, cr, uid, ids, vehicle_id, context=None):
        if not vehicle_id:
            return {}
        vehicle = self.pool.get('maint.vehicle').browse(cr, uid, vehicle_id, context=context)
    ...
    def _get_default_service_type(self, cr, uid, context):
        try:
            model, model_id = self.pool.get('ir.model.data').get_object_reference(cr, uid, 'maint', 'type_service_refueling')
        except ValueError:
            model_id = False
        return model_id

    _name = 'maint.vehicle.log.fuel'
    _description = 'Fuel log for vehicles'
    _inherits = {'maint.vehicle.cost': 'cost_id'}
    ...
    _columns = {
        ...
        'cost_id': fields.many2one('maint.vehicle.cost', 'Cost', required=True, ondelete='cascade'),
...
class maint_vehicle_log_services(osv.Model):
    ...
    def _get_default_service_type(self, cr, uid, context):
        try:
            model, model_id = self.pool.get('ir.model.data').get_object_reference(cr, uid, 'maint', 'type_service_service_8')
        except ValueError:
            model_id = False
        return model_id

    _inherits = {'maint.vehicle.cost': 'cost_id'}
    _name = 'maint.vehicle.log.services'
...
class maint_service_type(osv.Model):
    _name = 'maint.service.type'
...
class maint_vehicle_log_contract(osv.Model):
    ...
    def scheduler_manage_auto_costs(self, cr, uid, context=None):
        ...
        vehicle_cost_obj = self.pool.get('maint.vehicle.cost')
        d = datetime.datetime.strptime(fields.date.context_today(self, cr, uid, context=context), tools.DEFAULT_SERVER_DATE_FORMAT).date()
        contract_ids = self.pool.get('maint.vehicle.log.contract').search(cr, uid, [('state','!=','closed')], offset=0, limit=None, order=None,context=None, count=False)
        ...
        for contract in self.pool.get('maint.vehicle.log.contract').browse(cr, uid, contract_ids, context=context):
            ...
            while (startdate <= d) & (startdate <= datetime.datetime.strptime(contract.expiration_date, tools.DEFAULT_SERVER_DATE_FORMAT).date()):
                ...
                cost_id = self.pool.get('maint.vehicle.cost').create(cr, uid, data, context=context)
                startdate += deltas.get(contract.cost_frequency)
        return True
    ...
    def scheduler_manage_contract_expiration(self, cr, uid, context=None):
        ...
        for vehicle, value in res.items():
            self.pool.get('maint.vehicle').message_post(cr, uid, vehicle, body=_('%s contract(s) need(s) to be renewed and/or closed!') % (str(value)), context=context)
    ...
    def act_renew_contract(self, cr, uid, ids, context=None):
        ...
        for element in self.browse(cr, uid, ids, context=context):
            ...
            newid = super(maint_vehicle_log_contract, self).copy(cr, uid, element.id, default, context=context)
        mod, modid = self.pool.get('ir.model.data').get_object_reference(cr, uid, 'maint', 'maint_vehicle_log_contract_form')
        return {
            'name':_("Renew Contract"),
            'view_mode': 'form',
            'view_id': modid,
            'view_type': 'tree,form',
            'res_model': 'maint.vehicle.log.contract',
    ...
    def _get_default_contract_type(self, cr, uid, context=None):
        try:
            model, model_id = self.pool.get('ir.model.data').get_object_reference(cr, uid, 'maint', 'type_contract_leasing')
        except ValueError:
            model_id = False
        return model_id
    ...
    _inherits = {'maint.vehicle.cost': 'cost_id'}
    _name = 'maint.vehicle.log.contract'
    ...
    _columns = {
        ...
        'generated_cost_ids': fields.one2many('maint.vehicle.cost', 'contract_id', 'Generated Costs'),
...
class maint_contract_state(osv.Model):
    _name = 'maint.contract.state'
...
class facilities_detail(osv.Model):
    ...
    def _toilet_get_fnc(self, cr, uid, ids, prop, unknow_none, context=None):
        res = {}
        for record in self.browse(cr, uid, ids, context=context):
            istoilet = False
            if record.facility_type:
                if record.facility_type.name.lower().startswith('toilet') :
                    istoilet = True
            res[record.id] = istoilet
        return res
